// discover-recruitee.mjs — discover and verify Recruitee tenant slugs, end-to-end.
//
// Pipeline:
//   1. Query Common Crawl's URL index for *.recruitee.com patterns
//   2. Extract candidate slugs from matched URLs
//   3. Deduplicate and drop slugs already routed in data/companies_v2.json
//   4. Live-verify each candidate against https://{slug}.recruitee.com/api/offers/
//   5. Extract company name/website (from API response or board page)
//   6. Classify as gaming vs. tech using job titles
//   7. Output to output/recruitee-discovery-v2.json (matching lever-discovery-v2.json structure)
//
// Usage:
//   node discover-recruitee.mjs                 # all candidate slugs
//   node discover-recruitee.mjs --limit 100     # only verify 100 candidates
//   node discover-recruitee.mjs --use-cache     # re-filter cached results, no network
...
const RECRUITEE_CONFIG = {
  pattern: '*.recruitee.com',
  host: /^https?:\/\/([a-z0-9-]+)\.recruitee\.com/i,
  reserved: new Set([
    'www', 'app', 'api', 'blog', 'help', 'status', 'test', 'dev', 'demo',
    'careers', 'career', 'jobs', 'open', 'dashboard', 'admin', 'assets',
    'cdn', 'static', 'media', 'images', 'docs', 'support', 'mail', 'email',
    'staging', 'stage', 'stage2', 'dev2', 'qa', 'sandbox', 'preview',
  ]),
};
...
// ── Common Crawl fetching with retry/backoff ────────────────────────────────
...
function harvestSlugs(ndjson, into) {
  for (const line of ndjson.split('\n')) {
    ...
    if (!u) continue;
    const m = u.match(RECRUITEE_CONFIG.host);
    if (!m) continue;
    const slug = m[1].toLowerCase();
    if (RECRUITEE_CONFIG.reserved.has(slug)) continue;
    if (slug.length < 2) continue;
    into.add(slug);
  }
}

async function discoverFromCommonCrawl(crawls) {
  const slugs = new Set();
  for (const crawl of crawls) {
    let blocks;
    try {
      blocks = await blockCount(crawl, RECRUITEE_CONFIG.pattern);
    } catch (e) {
    ...
    for (let p = 0; p < blocks; p++) {
      try {
        harvestSlugs(await fetchBlock(crawl, RECRUITEE_CONFIG.pattern, p), slugs);
        ok++;
      } catch (e) {
...
async function probeTenant(url) {
  const r = await req(url, { json: true });
  // For Recruitee: 200 confirms a real tenant, redirects do not
  if (r.status === 200) return r;
  // If it's a redirect but not to a known non-tenant pattern, try following
  if (r.status >= 300 && r.status < 400 && r.location && !r.location.match(/^https?:\/\/(www\.)?recruitee\.com/i)) {
...
const NON_SITE = /recruitee\.com|linkedin|twitter|x\.com|facebook|instagram|youtube|tiktok|github\.com|glassdoor|w3\.org|schema\.org|googleapis|gstatic|amazonaws|cloudfront|google\.com|apple\.com|mailto:|javascript:|\.(png|jpg|jpeg|svg|gif|css|js|ico)(\?|$)/i;
...
// ── Verification ────────────────────────────────────────────────────────────
async function verifyRecruiteeTenant(slug) {
  const apiUrl = `https://${slug}.recruitee.com/api/offers/`;
  const r = await probeTenant(apiUrl);
  if (r.status !== 200) return null;
  const j = parseJson(r.body);
  if (!j || !Array.isArray(j.offers)) return null;
  if (j.offers.length === 0) return null; // Require at least one posting

  const jobs = j.offers.map(o => ({
    title: o.title || o.position || '',
    dept: o.department || '',
    location: [o.city, o.country].filter(Boolean).join(', ') || (o.locations && o.locations.length ? o.locations[0] : ''),
  }));

  // Try to extract company name from the API response or board page
  let name = slug, website = '';
  if (j.company_name) name = j.company_name;

  const board = await req(`https://${slug}.recruitee.com/`, { follow: true });
  if (board.status === 200) {
    const pageTitle = titleOf(board.body);
    const ogTitle = meta(board.body, 'og:title');
    if (pageTitle && !pageTitle.match(/recruitee/i)) name = pageTitle;
    else if (ogTitle && !ogTitle.match(/recruitee/i)) name = ogTitle;
    website = extractWebsite(board.body);
  }

  return { jobs, name, website, links: [apiUrl] };
}
...
const BAD_NAME = /\b(demo|sandbox|test|training|implementation|dummy|example|template|staging)\b.*\b(account|company|env|environment|board|site|tenant|[0-9]+)\b|^(lever|greenhouse|workable|breezy|bamboohr|teamtailor|recruitee)\b|\b(xml feed|rss feed|job feed|api feed|test company|your company here)\b/i;
...
function buildIndex() {
  ...
  for (const c of data) {
    ...
    for (const u of [c.website, ...(c.ats_links || []), ...(c.list_urls || [])]) {
      if (!u) continue;
      const m = String(u).match(recruiteeSlug);
      if (m && !['www', 'career', 'app', 'api'].includes(m[1].toLowerCase())) {
...
async function main() {
  ...
  console.error('discover-recruitee.mjs: end-to-end discovery & verification\n');

  // Step 1: Get recent crawl list
  const crawls = await listCrawls(N_CRAWLS);
  console.error(`Crawls: ${crawls.join(', ')}\n`);

  // Step 2: Discover from Common Crawl
  console.error('── Querying Common Crawl for *.recruitee.com');
  const raw = await discoverFromCommonCrawl(crawls);
  console.error(`  → ${raw.length} candidate slugs\n`);

  // Step 3: Load existing dataset and filter known slugs
  console.error('── Loading existing companies_v2.json');
  const index = buildIndex();
  let cands = raw.filter(s => !index.slugs.has(s.toLowerCase()) && !BAD_SLUG.test(s));
  const stats = {
    sources: ['commoncrawl-url-index'],
    raw: raw.length,
    after_known_slug: cands.length
  };
  console.error(`  → ${stats.raw} raw → ${stats.after_known_slug} unknown slugs\n`);

  cands = cands.slice(0, LIMIT);

  // Step 4: Verify against live API
  const cacheFile = path.join(OUT_DIR, 'verified-raw-recruitee.json');
  let verified;
  if (USE_CACHE && fs.existsSync(cacheFile)) {
    const c = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
    verified = c.verified;
    stats.verified_from_cache = c.verified_at;
    console.error(`[recruitee] ${stats.after_known_slug} unknown slugs → ${verified.length} from cache (${c.verified_at})`);
  } else {
    console.error(`── Live verification of ${cands.length} candidates (concurrency 20)`);
    verified = await pmap(cands, 20, async slug => {
      const v = await verifyRecruiteeTenant(slug);
      return v ? { slug, ...v } : null;
    }, (d, n) => process.stderr.write(`\r  ${d}/${n}`));
    process.stderr.write('\n');
    fs.writeFileSync(cacheFile, JSON.stringify({ ats: 'recruitee', verified_at: new Date().toISOString(), verified }));
  }
  stats.live_verified = verified.length;
  console.error(`  → ${stats.live_verified} verified\n`);

  // Step 5: Classify and filter
  console.error('── Classifying and filtering');
  const records = [];
  ...
  for (const v of verified) {
    ...
    records.push({
      ...
      _discovery: {
        ...
        gaming_score: cls.gaming_score,
        tech_score: cls.tech_score,
      },
    });
  }
  ...
  console.error(`  -${dropDup} duplicate | -${dropJunk} out-of-scope | → ${records.length} net-new\n`);

  // Step 6: Write output
  const out = path.join(OUT_DIR, 'recruitee-discovery-v2.json');
  ...
  console.error(`  Raw candidates from CC: ${stats.raw}`);
  console.error(`  Live verified: ${stats.live_verified}`);
  console.error(`  Net new (after dedup/filter): ${stats.net_new}`);
